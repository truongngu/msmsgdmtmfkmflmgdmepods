...
RectangleMesh::RectangleMesh(float width,float height,float uvOffsetX,float uvOffsetY)
{
	...
	int i = 0; int j = 0;
	verticesData[0].texcoord.x = i * dx;			verticesData[0].texcoord.y = 1 - j * dy; // 0 1
	verticesData[1].texcoord.x = i * dx;			verticesData[1].texcoord.y = 1 - j * dy - dy; // 0 0
	verticesData[2].texcoord.x = i * dx + dx;		verticesData[2].texcoord.y = 1 - j * dy - dy; // 1 0

	verticesData[3].texcoord.x = i * dx;			verticesData[3].texcoord.y = 1 - j * dy; // 0 1
	verticesData[4].texcoord.x = i * dx + dx;		verticesData[4].texcoord.y = 1 - j * dy; // 1 1
	verticesData[5].texcoord.x = i * dx + dx;		verticesData[5].texcoord.y = 1 - j * dy - dy; // 1 0
